...
export const Sidebar: React.FC = () => {
    ...
    return (
        <aside className="w-64 h-full bg-zinc-900/40 border-r border-zinc-800/60 flex flex-col shrink-0 no-print">
            {/* App Branding Area */}
            ...
            <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
                ...
                <NavItem
                    ...
                    onClick={() => console.log('Dashboard coming soon!')}
                />
                <NavItem
...
const NavItem: React.FC<NavItemProps> = ({ icon, label, isActive, onClick }) => {
    return (
        <button
            onClick={onClick}
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-md transition-all duration-200 text-sm font-medium focus:outline-none focus:ring-1 focus:ring-blue-500 ${isActive
